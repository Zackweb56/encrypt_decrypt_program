function encryptInput() {
  ...
  document.getElementById("encryptedInput").value = encryptedInput;
  document.querySelector(".copy_btn").textContent = "نسخ";
  document.querySelector(".copy_btn").style.cursor = "pointer";
  if (input === "") {
    document.getElementById("error_encrypt").textContent =
      "يجب عليك كتابة شيء ما أولا";
...
function vigenereCipher(text, key) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      ...
      // Lowercase a-z
      result += String.fromCharCode(
        ((charCode - 97 + (keyCharCode % 26)) % 26) + 97
      );
    } else {
...
function reverseVigenereCipher(text, key) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      // Uppercase A-Z
      result += String.fromCharCode(
        ((charCode - 65 - (keyCharCode % 26) + 26) % 26) + 65
      );
    } else if (charCode >= 97 && charCode <= 122) {
      // Lowercase a-z
      result += String.fromCharCode(
        ((charCode - 97 - (keyCharCode % 26) + 26) % 26) + 97
      );
    } else {
      result += text[i]; // Non-alphabetic characters remain unchanged
    }
...
async function copyText() {
  try {
      // Get the text field
      const copyText = document.getElementById("encryptedInput").value;
      
      // Use the Clipboard API to copy the text
      await navigator.clipboard.writeText(copyText);
      
      // Alert the copied text (optional)
      document.querySelector(".copy_btn").textContent = "تم النسخ";
      document.querySelector(".copy_btn").style.cursor = "not-allowed";
      // alert("لقد تم النسخ بنجاح" + copyText);
  } catch (err) {
